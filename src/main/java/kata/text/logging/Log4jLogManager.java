	...
	private void doConfigure(String defaultConfigFile, String defaultLogDirectory, String defaultLogFileName, boolean shouldWatch, TimeSpan watchPeriod)
	{
		try
		{
			...
			if (configUrlStr == null)
			{
				...
				{
					throw new KataException(AnagramExceptionCodes.LOGGING_CANNOT_FIND_LOGGING_CONFIGURATION_FILE, configFile.getAbsolutePath());
				}
			...
			else
			{
				...
				if (!configFile.exists())
				{
					throw new KataException(AnagramExceptionCodes.LOGGING_CANNOT_FIND_LOGGING_CONFIGURATION_FILE, configUrlStr);
				}
		...
		{
			throw new KataException(e, AnagramExceptionCodes.LOGGING_CANNOT_CONFIGURE_LOGGING_FRAMEWORK);
		}
	}
